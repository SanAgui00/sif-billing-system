		...
		<div class="col-sm-12">
			<div class="card mt-1">
					...
					<div class="card-header">
						<span class="title" id="menu-toggle">Mantener Catálogo Formas de pago</span> <span class="float-right text-secondary"
							id="btn-ocultar">Ocultar</span>
						<sf:form
						action="${pageContext.request.contextPath}/home/cat/sat/formaspago/crud"
						method="POST" name="new" id="nuevoElemento">
						<input name="event" type="hidden" value="new" />
						<button type="submit"
							class="btn-link float-right border-0 text-secondary">Nueva Forma de Pago</button>
					</sf:form>
					</div>
				<sf:form
					action="${pageContext.request.contextPath}/home/cat/sat/formaspago"
									...
									<div class="col-sm-8">
										<select class="ajax-form-negocio form-control form-control-sm"
											...
											<c:forEach items="${listaFormasPago}" var="lista">
												...
												${lista.id eq forma ? 'selected="selected"' : ''}>
		...
		<c:if test="${not empty consulta}">
			...
			<c:if test="${not empty registros_totales}">
				<sf:form
					...
					<input type="hidden" name="forma" id="getforma"
						class="globalGet">
