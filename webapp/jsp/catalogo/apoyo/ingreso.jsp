		...
		<div class="col-sm-12">
			<div class="card mt-1">
				<div class="card-header">
					<span class="title">Catálogo Categorías de Ingreso</span> <span class="float-right text-secondary" id="btn-ocultar">Ocultar</span>
					<sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/categorias/crud" method="POST" name="new" id="nuevoElemento">
						<input name="event" type="hidden" value="new" />
						<button type="submit" class="btn-link float-right border-0 text-secondary">Nueva Categoria</button>
					</sf:form>
				</div>
				<sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/categorias/consulta" method="POST" name="fuentes" autocomplete="off">
					<div id="mainMenu" class="collapse show card-body">
						<div class="form-row mt-2">
							<div class="form-group col-md-4 ">
								<label>Categoría de Ingreso: </label><select class="form-control form-control-sm" name="id">
									<option value="">Selecciona una opcion</option>
									<c:forEach items="${listaIngreso}" var="lista">
										<option value="<c:out value="${lista.id}"></c:out>"><c:out value="${lista.clave}"></c:out> - <c:out value="${lista.descripcion}"></c:out></option>
									</c:forEach>
								</select>
							</div>
		...
		<div class="col-sm-12">
			<c:if test="${not empty consulta}">
							...
							<tbody>
								<c:forEach items="${consulta}" var="consulta">
									<tr>
										...
										<td><sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/categorias/crud" method="POST" name="crud" class="text-center">
				...
				<c:if test="${not empty registros_totales}">
					<sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/categorias/" method="POST" name="pagination">
