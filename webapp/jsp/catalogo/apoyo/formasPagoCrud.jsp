		...
		<div class="card-body">
			<div class="form-row h-100">
					...
					<c:if test="${not empty consulta}">
						<c:forEach items="${consulta}" var="consulta">
							<c:if test="${event=='edit'}">
								<span class="title">Editando a <c:out value="${consulta.id_forma_pago}"></c:out></span>
							</c:if>
							<c:if test="${event=='delete'}">
								<span class="title">¿Quiere eliminar a <c:out value="${consulta.id_forma_pago}"></c:out> ?
								</span>
							</c:if>
						</c:forEach>
					</c:if>
				...
				<div class="form-group col-md-6 auto">
					<c:if test="${event=='delete'}">
						<sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/formasdepago/save" method="POST" name="insert">
							<c:forEach items="${consulta}" var="consulta">
					...
					<a class="btn btn-secondary text-white btn-sm float-right " href="<c:url value='/home/cat/apoyo/formasdepago'/>" role="button">Regresar</a>
				</div>
			</div>
		</div>
	</div>
