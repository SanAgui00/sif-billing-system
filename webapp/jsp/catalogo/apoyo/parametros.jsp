		...
		<div class="col-sm-12">
			<div class="card mt-1">
				<div class="card-header">
					<span class="title">Catálogo de Parámetros</span> <span class="float-right text-secondary" id="btn-ocultar">Ocultar</span>
					<sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/parametros/crud" method="POST" name="new" id="nuevoElemento">
						<input name="event" type="hidden" value="new" />
						<button type="submit" class="btn-link float-right border-0 text-secondary">Nuevo Parámetro</button>
					</sf:form>
				</div>
				<sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/parametros/consulta" method="POST" autocomplete="off">
		...
		<div class="col-sm-12">
			<c:if test="${not empty consulta}">
							...
							<tbody>
								<c:forEach items="${consulta}" var="consulta">
									<tr>
										...
										<td><c:out value="${consulta.valor1}"></c:out></td>
										<td><c:out value="${consulta.valor2}"></c:out></td>
										<td><c:out value="${consulta.estatus}"></c:out></td>
										<td><c:out value="${consulta.usuario}"></c:out></td>
										<td><c:out value="${consulta.fecha}"></c:out></td>
										<td><sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/parametros/crud" method="POST" name="crud" class="text-center">
				...
				<c:if test="${not empty registros_totales}">
					<sf:form action="${pageContext.request.contextPath}/home/cat/apoyo/parametros/consulta" method="POST" name="pagination">
